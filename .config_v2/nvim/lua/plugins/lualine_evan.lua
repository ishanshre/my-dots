return {
  ...
  opts = function()
    ...
    local conditions = {
      ...
      hide_in_width_first = function() return vim.fn.winwidth(0) > 80 end,
      hide_in_width = function() return vim.fn.winwidth(0) > 70 end,
    ...
    local function active_left(component) table.insert(config.sections.lualine_c, component) end
    ...
    local function active_right(component) table.insert(config.sections.lualine_x, component) end
    ...
    active_left {
      function()
        ...
        if ok then
          ...
          if icon == nil then icon = devicons.get_icon_by_filetype(vim.bo.filetype) end
        else
      ...
      function()
        if vim.lsp.get_clients then
          ...
          for _, client in pairs(clients) do
            if not clients_list[client.name] then table.insert(clients_list, client.name) end
          end
